...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Bollywoodcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Bollywoodcoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        Object result;
        result.push_back(Pair("data",     HexStr(BEGIN(pdata), (char *) &pdata[21])));
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
