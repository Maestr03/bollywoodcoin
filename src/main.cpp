...
#if defined(NDEBUG)
# error "Bollywoodcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x94f6260a464212221fd90b900159b29982277af5a5a0b95ed6f08e2c6f7e0266");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20);
// Starting difficulty
static CBigNum bnGenesisBlockDiff(~uint256(0) >> 28);
...
const string strMessageMagic = "Bollywoodcoin Signed Message:\n";
...
{
    // Bollywoodcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1) nSubsidy = 10000000 * COIN;
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Bollywoodcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Bollywoodcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (pindexLast->nHeight+1 == 1)
        return bnGenesisBlockDiff.GetCompact();
    ...
    // Bollywoodcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1ca8baf3ebcd7fbe7f929964d6f9b02ad7fb99fab8f6e27cec59cbe1d4a99e74");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "TOI 17/11/14 British parliament evacuated over suspicious package";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1416239000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 339735844;

        if (fTestNet)
        {
            block.nTime    = 1416239100;
            block.nNonce   = 3062702556;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xcd252671cb4107baf3941317da595ff39f1cd58814f48fceda3ed9e0412726ec"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x1b, 0xd6, 0xe9, 0x01 }; // Bollywoodcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BollywoodcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BollywoodcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BollywoodcoinMiner(CWallet *pwallet)
{
    printf("BollywoodcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bollywoodcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BollywoodcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BollywoodcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BollywoodcoinMiner, pwallet));
}
